...
def test_sample_complexity(sim_conversion_training_config, save_dir: str, generate_results: bool=True):
    ...
    if generate_results:
        ...
        for train_sample_size in train_sample_sizes:
            ...
            copy_sim_conversion_training_config = deepcopy(sim_conversion_training_config)
            ...
            for model_name, model_config in ml_stat_comparison_model_configs(width=retrieve_image_width_from_settings(sim_conversion_training_config)).items():
                ...
                config = {
                    'train': copy_sim_conversion_training_config.copy(), # ensure train sample size is set correctly
                    'test': sim_conversion_training_config.copy(), # ensure test sample size is set correctly
                    'model': model_config.copy()
                }
                model = retrieve_model(config)(config.copy())
                add_to_accs_dict(data_accs, model_name, model.test(accuracy_value=True,test_in_batches=True)[0])
            ...
            for stat_name, stat_config in stat_comparison_configs().items():
                ...
                acc = stat_model.test(accuracy_value=True,test_in_batches=True)[0]
                add_to_accs_dict(data_accs, stat_name, acc)

        # Convert sample complexity to actual training size
        sample_sizes = [int(sample_size*0.8) for sample_size in train_sample_sizes]

        # store results in csv
        csv_filename = os.path.join(save_dir, 'sample_complexity.csv')
        with open(csv_filename, 'w') as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(['Training Set Size'] + [name for name in data_accs])
            for i, sample_size in enumerate(sample_sizes):
                writer.writerow([sample_size] + [accs[i] for _,accs in data_accs.items()])
