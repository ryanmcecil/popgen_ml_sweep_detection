...
from allel import read_vcf, sfs_folded
from matplotlib import pyplot as plt

from reproduce.article.sfs.compute_mean_std import *
...
if __name__ == '__main__':
    ...
    plot_name = os.path.join(
        os.getcwd(), 'reproduce/reproduce/results/sfs', f'neutral_sfs_folded_plot.png')
    ...
    for sim in sims:
        ...
        simulator = retrieve_simulator(
            sim['settings']['software'])(sim['settings'])
        counts = np.zeros(length)
        for i in range(N):
            id = i + 1
            image = simulator.load_data(
                id_num=id, datatype=sim['datatype']).astype(np.int)
